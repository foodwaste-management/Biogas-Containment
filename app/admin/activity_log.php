...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
  $filename = "admin_activity_log_" . date('Ymd_His') . ".csv";
  header("Content-Type: text/csv");
  header("Content-Disposition: attachment; filename=\"$filename\"");
  $out = fopen('php://output', 'w');
  fputcsv($out, ['ID', 'User', 'Activity', 'Type', 'IP Address', 'Time']);
  $stmt = $db->prepare($sql);
  $stmt->execute($params);
  while ($r = $stmt->fetch()) {
    fputcsv($out, [$r['id'], $r['email'] ?? 'System', $r['activity'], $r['activity_type'], $r['ip_address'], $r['created_at']]);
  }
  fclose($out);
  exit;
}

$stmt = $db->prepare($sql . " LIMIT 200");
$stmt->execute($params);
$logs = $stmt->fetchAll();
          ...
          <div class="table-wrap">
            <table class="data-table">
              ...
              <tbody>
                ...
                <?php if (!$logs): ?>
                  <tr>
                    <td colspan="6" class="text-center text-muted" style="padding:32px">No activity logged yet.</td>
                  </tr>
                <?php endif; ?>
              </tbody>
            </table>
